/* =========================================================
   HEALTHY FOOD EXPLORER SCRIPT (healthy.js)
   ========================================================= */
...
function searchFood() {
    ...
    if (!query && !category) {
        if (resultsContainer) loadDefaultMeals(resultsContainer);
        return;
    }
...
async function fetchFoodData(query, category, container) {
    ...
    try {
        const response = await fetch(`/api/foods?search=${encodeURIComponent(query)}&category=${encodeURIComponent(category)}`);
        const data = await response.json();
